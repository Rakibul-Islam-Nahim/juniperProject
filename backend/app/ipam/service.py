...
(default /24) and one block is allocated per lab. The very first usable address of each
block is the gateway; `.10` is the microVM management IP.

Allocation is transactional and protected against races via row-level locks. Releases
soft-delete the row (sets `released_at`) and free the block for reuse.
"""
...
class IPAMService:
    ...
    async def allocate(self, db: AsyncSession, lab_id: str) -> AllocationResult:
        ...
        Uses a row lock on existing allocations to avoid races when two labs request
        concurrently.
        """
        # Row-lock existing allocations so concurrent allocates serialise.
        await db.execute(
            select(IPAllocation).with_for_update()
        )

        taken = await db.execute(select(IPAllocation.subnet).where(IPAllocation.released_at.is_(None)))
    ...
    async def release(self, db: AsyncSession, lab_id: str) -> None:
        await db.execute(
            ...
            .values(released_at=__import__("datetime").datetime.now(__import__("datetime").timezone.utc))
        )
        await db.flush()

    async def hard_release(self, db: AsyncSession, lab_id: str) -> None:
        """Used during destruction — deletes the row outright."""
    ...
    async def get(self, db: AsyncSession, lab_id: str) -> IPAllocation | None:
        result = await db.execute(
            select(IPAllocation).where(IPAllocation.lab_id == lab_id, IPAllocation.released_at.is_(None))
        )
        return result.scalar_one_or_none()

    async def list_active(self, db: AsyncSession) -> Iterable[IPAllocation]:
        result = await db.execute(select(IPAllocation).where(IPAllocation.released_at.is_(None)))
